...
const WS_URL = "ws://" + window.location.hostname + ":3005";
...
async function handlePinSubmit() {
    ...
    try {
        ...
        if (!res.ok) throw new Error(data.message || "Mã PIN không chính xác");
...
// ================= UTILS (Keep existing ones) =================
// [Existing code for WebSocket, Bank QR, Polling etc continues...]
...
function renderBankQR(bankInfo) {
    ...
    if (!bankInfo) bankInfo = { account_no: "131020047979", amount: amount, content: bookingCode, qr_url: `https://img.vietqr.io/image/970422-131020047979-compact2.jpg?amount=${amount}&addInfo=${encodeURIComponent(bookingCode)}&accountName=NGUYEN%20VAN%20DUY` };
    const qrImg = document.getElementById("bank-qr-img");
    if (qrImg) { qrImg.style.display = "block"; qrImg.src = bankInfo.qr_url; }
    setText("bk-acc", bankInfo.account_no || "131020047979");
    setText("bk-amount", formatPrice(amount));
    setText("bk-content", bankInfo.content || bookingCode);
}
