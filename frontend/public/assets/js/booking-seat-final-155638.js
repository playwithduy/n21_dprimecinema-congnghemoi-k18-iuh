...
// Cố gắng sử dụng hostname hiện tại nếu là localhost/127.0.0.1 để tránh lỗi mismatch IPv4/IPv6
const getWsUrl = () => {
    let url = window.APP_CONFIG.WS_URL;
...
async function loadShowtimeInfo() {
    ...
    try {
        ...
        if (moviePosterEl && data.poster) {
            ...
            // API_BASE là ".../api", nên ta cần lấy root (bỏ /api)
...
function connectSocket() {
    ...
    socket.onmessage = (msg) => {
        ...
        if (data.type === "INIT") {
            console.log(`🎬 INIT RECEIVED: ${data.seats?.length || 0} seats`);
            isInitLoading = true;
...
function renderSeats() {
    console.log("🛠️ Rendering seats with state:", Object.keys(seatState).length, "keys");
    if (!seatMap) {
    ...
    console.log("📊 Rows identified:", rowKeys);
...
function updateText() {
    ...
    selected.forEach(code => {
        ...
        if (seatPrices[type]) total += seatPrices[type];
    });
...
if (nextBtn) {
    nextBtn.addEventListener("click", () => {
        ...
        selected.forEach(code => {
            const type = seatState[code]?.seat_type;
            if (seatPrices[type]) seatTotal += seatPrices[type];
        });
